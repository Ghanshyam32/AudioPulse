package com.ghanshyam.spotifyclone.exoplayer
...
class FirebaseMusicSource @Inject constructor(
    private val musicDatabase: MusicDB
){
    ...
    fun asMediaSource(dataSourceFactory: DefaultDataSourceFactory): ConcatenatingMediaSource {
        ...
        songs.forEach { songs ->
            val mediaSource = ProgressiveMediaSource.Factory(dataSourceFactory)
                .createMediaSource(songs.getString(METADATA_KEY_MEDIA_URI).toUri())
            concatenatingMediaSource.addMediaSource(mediaSource)
        }
        return concatenatingMediaSource

    }
    fun asMediaItems() = songs.map { songs ->
        val desc = MediaMetadataCompat.Builder()
